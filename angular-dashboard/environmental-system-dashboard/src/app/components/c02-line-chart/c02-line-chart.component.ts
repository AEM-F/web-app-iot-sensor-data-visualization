...
@Component({
  selector: 'app-c02-line-chart',
  templateUrl: './c02-line-chart.component.html',
  styleUrls: ['./c02-line-chart.component.css']
})
export class C02LineChartComponent implements OnInit {

  @Input() public data: SensorValue[] = [];
  private label:string =  "C02";
  private backgroundColor:string = "rgba(255,119,0,0.3)";
  private borderColor:string = "rgba(255,102,0,0.9)";
  ...
  public dataC02:ChartConfiguration['data'] = {
    datasets:[
      {
        data:[],
        label: this.label,
  ...
  private UpdateData(){
    if(this.data.length >0){
      ...
      for (let datum of this.data) {
        this.pushOne(datum.c02,datum.sample_time.toISOString());
      }
    }
  }

  public pushOne(value:number, label:string): void {
    this.dataC02.datasets.forEach((x, i) => {
      x.data.push(value);
    });
    this.dataC02?.labels?.push(label);
