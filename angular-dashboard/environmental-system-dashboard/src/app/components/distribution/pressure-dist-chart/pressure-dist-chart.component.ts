...
@Component({
  selector: 'app-pressure-dist-chart',
  templateUrl: './pressure-dist-chart.component.html',
  styleUrls: ['./pressure-dist-chart.component.css']
})
export class PressureDistChartComponent implements OnInit {
    ...
    datasets: [
      {
        ...
        borderColor: "rgba(179,0,255,0.6)",
        hoverBackgroundColor:"rgba(187,0,255,0.9)",
        hoverBorderColor: "rgba(166,0,255,0.6)"
      }
    ]
  };
  ...
  private UpdateData(){
    if(this.data.length >0){
      ...
      const countOccurrences = (arr:SensorValue[], val:number) => arr.reduce((a, v) => (v.pressure === val ? a + 1 : a), 0);
      for (let datum of this.data) {
        this.pushOne(datum.pressure,countOccurrences(this.data, datum.pressure));
      }
    }
  }
