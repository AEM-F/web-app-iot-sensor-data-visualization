...
@Component({
  selector: 'app-all-values-line-chart',
  templateUrl: './all-values-line-chart.component.html',
  styleUrls: ['./all-values-line-chart.component.css']
})
export class AllValuesLineChartComponent implements OnInit {
  ...
  private UpdateData(){
    if(this.data.length >0){
      ...
      this.dataTemp.datasets[2].data = [];
      this.dataTemp.datasets[3].data = [];
      ...
      for (let datum of this.data) {
        this.pushOne(datum.temperature,datum.humidity,datum.c02,datum.tvoc,datum.sample_time.toISOString());
      }
    }
  }

  public pushOne(valueTemp:number,valueHum:number, valueC02:number,valueTvoc:number,label:string): void {
    this.dataTemp.datasets[0].data.push(valueTemp);
    this.dataTemp.datasets[1].data.push(valueHum);
    this.dataTemp.datasets[2].data.push(valueC02);
    this.dataTemp.datasets[3].data.push(valueTvoc);
